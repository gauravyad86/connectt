...
import { View, Text, Image, FlatList, TouchableOpacity, ScrollView, Dimensions, StyleSheet } from 'react-native';
...
const Home2 = ( props ) => {
    ...
    const handleNavigate = (direction) => {
        ...
        if (direction === 'next') {
            ...
            newIndex = (currentIndex - 1 + Data.parentUsers.length) % Data.parentUsers.length;  // Move to the previous user (circular)
        }
    ...
    const handleTileNavigation = ( type ) => {
        if ( type === 'next' ) {
            if ( userType === 'parent' && Data.parentUsers[ currentIndex ].subParents.length ) {
                setUserType( 'subParent' );
            } else if ( userType === 'subParent' && Data.parentUsers[ currentIndex ].children.length ) {
                setUserType( 'child' );
            }
        } else if ( type === 'previous' ) {
            if ( userType === 'child' && Data.parentUsers[ currentIndex ].subParents.length ) {
                setUserType( 'subParent' );
    ...
    return (
        <View style={ { flex: 1, marginTop: 10 } }>
            ...
            { view === 'list' ? (
                <ListView users={ Data.parentUsers } onSelectUser={ ( index ) => {
...
const ListView = ( { users, onSelectUser } ) => (
    <ScrollView vertical style={ styles.users }>
        { users.map( ( user, index ) => (
            <TouchableOpacity key={ index } onPress={ () => onSelectUser( index ) } style={ { marginRight: 20, alignItems: 'center' } }>
...
const TileView = ({
    user, userType, onNavigate, onTileNavigation, onBackToList,
}) => { 
